...
def get_k_projections(x, a):
    ...
    phi = 2. - 4 * R + np.sqrt(5. - 16 * R + 16 * R ** 2) # Rayleigh
...
def atom_scattering(x, k, T, vbulk):
    ...
    upar = get_upar(x, a)
    uperp = get_uperp()
    r1, r2, mu = get_k_projections(x, a)
    ...
    vth = get_vth(T)
    g = get_g(T)
    xout = x + (mu - 1.) * upar + r1 * uperp[0] + r2 * uperp[1] + numba_dot(vbulk / vth, kout) + g * (mu - 1.)
    return xout, kout
...
def runner(Nphotons=1000):
    """
    emission from center of a sphere of radius RSIZE 
    expanding with outer edge velocity VMAX at line center x = 0
    """
